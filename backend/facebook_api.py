...
class FacebookAPI:
    ...
    def _get_page_token(self, page_id: str) -> str:
        """
        Cache & return PAGE access token. Falls back to fresh /me/accounts fetch.
        ...
        Raises:
            ValueError: If page not found or token missing permission
        """
        ...
        if page_id not in self._page_token_cache:
            raise ValueError(f"Page {page_id} not found or token missing permission.")
    ...
    def publish_post(self, page_id: str, message: str, link: Optional[str] = None) -> str:
        ...
        Raises:
            FacebookAPIError: If publication fails
        ...
        try:
            ...
            logger.error(f"Page token error for page {page_id}: {e}")
            raise FacebookAPIError(f"Page token error: {e}")
        except Exception as e:
            logger.error(f"Failed to publish post to page {page_id}: {e}")
            raise FacebookAPIError(f"Publication failed: {e}")
    ...
    def publish_post_with_photos(self, page_id: str, message: str, files: List[str]) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to publish post with photos to page {page_id}: {e}")
            raise FacebookAPIError(f"Publication with photos failed: {e}")
    ...
    def publish_post_with_video(self, page_id: str, video_path: str, message: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to publish video to page {page_id}: {e}")
            raise FacebookAPIError(f"Video publication failed: {e}")
    ...
    def publish_post_with_photo_ids(self, page_id: str, message: str, photo_ids: List[str], 
    ...
    # Note: _get_page_token method is defined earlier in the class
