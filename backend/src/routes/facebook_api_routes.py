...
def bulk_publish():
    ...
    try:
        ...
        for pid in page_ids:
            try:
                if video:
                    pid_post = api.publish_post_with_video(pid, video, message)
                elif images:
                    pid_post = api.publish_post_with_photos(pid, message, images)
                else:
                    pid_post = api.publish_post(pid, message, link)
                results[pid] = {"status": "ok", "post_id": pid_post}
            except Exception as e:
                logger.exception("Publish failed for %s", pid)
...
def publish():
    """
    Publish content to Facebook pages (simplified route)
    """
    try:
        data = request.get_json()
        api = get_facebook_api()
        ...
        current_app.logger.error(f"Error in publish: {e}")
        return jsonify({'error': 'Internal server error'}), 500
...
@facebook_bp.route('/test-connection', methods=['GET'])
def test_connection():
    ...
    try:
        ...
        response = api._make_request("GET", "/me", params={"fields": "id,name"})
        ...
        if response and 'id' in response:
            ...
            return jsonify({
                ...
                'message': f'Connexion réussie ! {len(pages)} pages trouvées.'
            })
        else:
            return jsonify({
                'success': False,
        ...
        return jsonify({
            'success': False,
            'error': f'Erreur Facebook API: {str(e)}'
        }), 400
    except Exception as e:
